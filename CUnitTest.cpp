...
do ordain and establish this constitution of the United States of America\n\n");
	...
	bool GRAYCALL CUnitTestCur::IsTestInteractive() // static
	{
	...
	bool GRAYCALL CUnitTestCur::IsInMSTest() // static 
	{
		// Just because it was compiled for M$ Test doesn't mean I am running it that way.
		return !sm_bCreatedUnitTests && sm_nCreatedUnitTests > 0;
	}
#endif

	bool GRAYCALL CUnitTestCur::TestInteractivePrompt(const char* pszMsg) // static
	{
		...
		bool bRet = false;
#ifdef _WIN32
		int iRet = _GTF(::MessageBox)(WINHANDLE_NULL, StrArg<GChar_t>(pszMsg), _GT("Gray CUnitTest"), MB_OKCANCEL);
		bRet = (iRet == IDOK);
#else	// __linux__
		// console.
	...
	CUnitTestRegister* CUnitTests::FindUnitTest(const char* pszName) const
	{
		...
		for (ITERATE_t i = 0; i < m_aUnitTests.GetSize(); i++)
		{
			if (!StrT::CmpI(m_aUnitTests[i]->m_pszTestName, pszName))
				return m_aUnitTests[i];
		}
		return nullptr;
	}
	...
	bool CUnitTests::TestTypes()
	{
		...
		// CASSERT(sizeof(void) == 0, void);
		CASSERT(sizeof(char) == 1, char);
		CASSERT(sizeof(short) == 2, short);
		CASSERT(sizeof(INT32) == 4, INT32);
		CASSERT(sizeof(UINT32) == 4, UINT32);
		CASSERT(sizeof(CUnion32) == 4, CUnion32);	// 32 bits exactly.
		CASSERT(sizeof(INT64) == 8, INT64);
		CASSERT(sizeof(UINT64) == 8, UINT64);
		CASSERT(sizeof(CUnion64) == 8, CUnion64);	// 64 bits exactly.
		CASSERT(sizeof(DWORD) == 4, DWORD);			// _MSC_VER 64=4, _MSC_VER 32=4
...
#ifdef _WIN32
		CASSERT(true == TRUE, TRUE);	// _WIN32 only
		bool bVal = true;
