...
namespace Gray
{
	...
	STREAM_SEEKRET_t CFileTextReader::Seek(STREAM_OFFSET_t iOffset, SEEK_ORIGIN_TYPE eSeekOrigin) // override;
	{
		...
		case SEEK_Set: // from beginning.
			...
			if (iOffset >= nPosFile - get_WriteIndex() && iOffset <= nPosFile)
			{
				// Move inside buffer.
...
UNITTEST_CLASS(CFileTextReader)
{
	UNITTEST_METHOD(CFileTextReader)
	{
		...
		CStringF sFilePath = CFilePath::CombineFilePathX(get_TestInpDir(), _FN("GrayCore/CFileTextReader.cpp"));

		static const int k_MaxLineLen = 128;	// CStream::k_FILE_BLOCK_SIZE 256
		...
		for (;; )
		{
			...
			//*** Make this over 128 chars long ****************************************************************************************************************************************************

			if (hRes >= k_MaxLineLen)
			{
				// Warning  line length was too long !
				UNITTEST_TRUE(iLines == 166);	// Fix this if source changes.
				DEBUG_MSG(("line %d length was > %d", iLines, hRes));
			}
			else
			{
				iLines++;	// don't count split lines.
			}

			UNITTEST_TRUE(hRes <= k_MaxLineLen);
		}

		UNITTEST_TRUE(iLines == 187);	// Fix this if source changes.
	}
};
