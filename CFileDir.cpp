...
namespace Gray
{
	...
	UINT GRAYCALL cFileDevice::GetDeviceType(const FILECHAR_t* pszDeviceId)
	{
		...
		//! @return 0 = unknown, 1=Bad Path, 2=removable, 3=fixed, 4=remote, 5=CDROM, 6=RAM
#ifdef _WIN32
...
#error NOOS
	...
	HRESULT CFileDir::ReadDir(const FILECHAR_t* pszDirPath, const FILECHAR_t* pszWildcardFile, ITERATE_t iFilesMax, bool bFollowLink)
	{
		...
		//! @arg
		//!  pszDirPath = the base directory.
		//!  pszWildcardFile = may use a filter like: *.scp. may contain full path if pszDirPath is empty.
...
UNITTEST_CLASS(CFileDir)
{
	HRESULT TestDevice(cFileDevice& di, const FILECHAR_t* pszDeviceId, CStringL sNameDisplay)
	{
		...
		UNITTEST_TRUE(uFreeDiskSpace > 0);
	...
	void TestReadDir()
	{
		CStringF sTestInpDir = CFilePath::CombineFilePathX(get_TestInpDir(), _FN(GRAY_NAMES) _FN("Lib"));

		const int s_nMinFiles = 4; // min number of header files assumed to be in this dir.
		CFileDir fd;
