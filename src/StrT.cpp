...
GRAYCORE_LINK StrLen_t GRAYCALL StrT::vsprintfN<char>(cSpanX<char> ret, const char* pszFormat, va_list vlist) {
    ...
    return ::vsnprintf(ret.get_PtrWork(), ret.GetSize(), pszFormat, vlist);  // C99
#elif USE_CRT && (_MSC_VER >= 1400) && !defined(UNDER_CE)
    // CRT version. act as _TRUNCATE
    return ::_vsnprintf_s(ret.get_PtrWork(), (size_t)(ret.GetSize()), (ret.GetSize() - 1), pszFormat, vlist);  // to shut up the deprecated warnings.
#elif USE_CRT
    // OLD CRT version.
    return ::_vsnprintf(ret.get_PtrWork(), ret.GetSize(), pszFormat, vlist);
#else  // _WIN32
...
#if defined(__linux__)
    return ::vswprintf(ret.get_PtrWork(), ret.GetSize(), pszFormat, vlist);  // C99
#elif USE_CRT && (_MSC_VER >= 1400) && !defined(UNDER_CE)
    // CRT version. act as _TRUNCATE
    return ::_vsnwprintf_s(ret.get_PtrWork(), (size_t)(ret.GetSize()), (size_t)(ret.GetSize() - 1), pszFormat, vlist);  // to shut up the deprecated warnings.
#elif USE_CRT
    // OLD CRT version.
    return ::_vsnwprintf(ret.get_PtrWork(), ret.GetSize(), pszFormat, vlist);
#else  // _WIN32
       // Use internal version of _vsnprintf
