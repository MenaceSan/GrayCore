 ...
 public:
    ...
    HRESULT addEvent(cLogEvent& rEvent) noexcept override {
        if (m_File.isValidHandle()) {
            m_File.WriteString(rEvent.get_FormattedDefault());
        }
        return 1;
    }
};
...
#undef k_abc
    ...
    STATIC_ASSERT(sizeof(pRefObj) == sizeof(void*) + sizeof(cPtrTrace), cRefPtr<>);
#else
    STATIC_ASSERT(sizeof(pRefObj) == sizeof(void*), cRefPtr<>);
#endif
...
cUnitTestRegister::cUnitTestRegister(const TYPEINFO_t& rTypeInfo, UNITTEST_LEVEL_t nTestLevel) : cObjectFactory(rTypeInfo), m_nTestLevel(nTestLevel) {
...
HRESULT cUnitTests::InitTestOutDir() {
    ...
    for (; SUCCEEDED(hRes); hRes = dir1.FindFileNext()) {
        ...
        if (dir1.isDots()) continue;
        if (dir1.m_FileEntry.get_Name().StartsWithI(_FN(GRAY_NAMES))) continue;
    ...
    if (hRes == HRESULT_WIN32_C(ERROR_NO_MORE_ITEMS)) return nCount;
    return hRes;
}
...
bool cUnitTests::IsTestInteractive() const noexcept {
    ...
    if (m_nTestLevel <= UNITTEST_LEVEL_t::_Crit) return true;
    if (m_nTestLevel >= UNITTEST_LEVEL_t::_All) return true;
#if 0  // def _DEBUG
...
HRESULT cUnitTests::RunUnitTests(UNITTEST_LEVEL_t nTestLevel, const LOGCHAR_t* pszTestNameMatch) {
    ...
    for (ITERATE_t i = 0; i < m_aUnitTests.GetSize(); i++) {
        ...
        m_pLog->addDebugInfoF("cUnitTest '%s' run  %s", LOGSTR(pUnitTest->m_pszTypeName), szDashes);
        ...
        if (m_iFailures > 0) {
            ...
            m_pLog->addDebugInfoF("cUnitTest FAILED '%s' Test %d", LOGSTR(pUnitTest->get_Name()), iTestsRun);
            return E_FAIL;
        }
        ...
        if (!bHeapCheck && !::Gray::cDebugAssert::Assert_Fail("cHeap::Check", DEBUGSOURCELINE)) {
            // Assume cDebugAssert::Assert_Fail was also called.
            m_pLog->addDebugInfoF("cUnitTest FAILED '%s' Heap Corruption.", LOGSTR(pUnitTest->get_Name()));
            return E_FAIL;
        }
        ...
        m_pLog->addDebugInfoF("cUnitTest '%s' complete in %s %s", LOGSTR(pUnitTest->get_Name()), LOGSTR(sTimeSpan), szDashes);
        szDashes[_countof(szDashes) - nLenText] = '-';
    }

    m_bRunning = false;

    m_pLog->addDebugInfoF("cUnitTests ENDING %d/%d TESTS in %s", iTestsRun, m_aUnitTests.GetSize(), LOGSTR(cTimeInt::GetTimeSpanStr(tStart.get_AgeSec())));
