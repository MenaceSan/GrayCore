...
namespace Gray
{
	...
	template< typename _TYPE_CH>
	HRESULT cStringT<_TYPE_CH>::ReadZ(CStreamInput& File, StrLen_t iLenMax)
	{
		...
		HRESULT hRes = File.ReadX(pBuffer, iLenMax);
...
#if 1 // def GRAY_DLL // force implementation/instantiate for DLL/SO.
#ifndef _MFC_VER
...
{
	template< typename _TYPE_CH>
	void GRAYCALL cStringT<_TYPE_CH>::UnitTestT() // static
	{
		// k_asTextLines
...
UNITTEST_CLASS(cString)
{
	...
	{
		cStringT<char>::UnitTestT();
		cStringT<wchar_t>::UnitTestT();
	}
};
UNITTEST_REGISTER(cString, UNITTEST_LEVEL_Core);
