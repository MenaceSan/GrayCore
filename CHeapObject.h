...
namespace Gray
{
	DECLARE_INTERFACE(IHeapObject)
	{
		...
		virtual const void* get_HeapPtr() const = 0;	//!< Get the top level (outermost, freeable) class pointer. I can delete get_HeapPtr().

		// Add this to each IHeapObject rooted object to get the base heap allocation pointer. Avoid problems with multiple inheritance.
#define CHEAPOBJECT_IMPL virtual const void* get_HeapPtr() const override { return this; }
	};
