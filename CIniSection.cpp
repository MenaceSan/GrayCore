...
namespace Gray
{
	...
	HRESULT CIniSectionData::ReadSectionData(cStringA& rsSectionNext, CStreamInput& stream, bool bStripComments)
	{
		...
		for (;;)
		{
			...
			// Read a line.
			hRes = stream.ReadStringLine(pszLine, dwLenBufferMax - i);
			if (FAILED(hRes))
				break;
