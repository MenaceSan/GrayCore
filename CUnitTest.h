...
namespace Gray
{
	...
	struct GRAYCORE_LINK CUnitTestCur	// static
	{
		...
		static bool GRAYCALL IsTestInteractive();
		static bool GRAYCALL TestInteractivePrompt(const char* pszMsg);

#if defined(USE_UNITTESTS_MS)
		static bool GRAYCALL IsInMSTest();
#endif

		static const FILECHAR_t* GRAYCALL get_TestOutDir();
		static const FILECHAR_t* GRAYCALL get_TestInpDir();

		static CStringF sm_sTestInpDir;
		static CStringF sm_sTestOutDir;
	...
	public:
		...
		virtual ~CUnitTest(); // noexcept(false); // Needed for vs2017
#else
		virtual ~CUnitTest();
#endif
