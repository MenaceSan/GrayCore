...
namespace Gray
{
	...
	HRESULT CStreamOutput::WriteStream(CStreamInput& sInp, STREAM_POS_t nSizeMax, IStreamProgressCallback* pProgress, TIMESYSD_t nTimeout)
	{
		//! Copy data from one read stream (sInp) to another write stream (this). 
		...
		for (; dwAmount < nSizeMax;)
		{
			...
			HRESULT hResRead = sInp.ReadX(Data.get_Data(), nSizeBlock);
			if (FAILED(hResRead))
			{
...
void CStream::UnitTest_StreamIntegrity(CStreamOutput* pStreamOut, CStreamInput* pStreamInp, size_t nSizeTotal)
{
	...
	for (;; i++)
	{
		...
		if (iSizeWriteTotal < nSizeTotal)	// write more?
		{
			...
			hRes = pStreamOut->WriteX(blockwrite.get_DataBytes() + (iSizeWriteTotal%iSizeBlock), iSizeWriteBlock);
		...
		hRes = pStreamInp->ReadX(pRead, iSizeReadBlock);
		UNITTEST_TRUE(SUCCEEDED(hRes));
		nSizeReal = (size_t)hRes;
