...
namespace Gray
{
	...
	HRESULT CIniFile::ReadIniStream(CStreamInput* pInput, bool bStripComments)
	{
		...
		for (;;)
		{
			...
			HRESULT hRes = pInput->ReadStringLine(szBuffer, STRMAX(szBuffer));
...
const FILECHAR_t* CIniFile::k_UnitTestFile = _FN("GrayCore") _FN(FILESTR_DirSep) _FN("CIniFileUnitTest") _FN(MIME_EXT_ini);
