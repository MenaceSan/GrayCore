...
#ifdef USE_UNICODE
	typedef wchar_t GChar_t;		//!< My version of TCHAR
#define _GT(x)		__TOW(x)	//!< like _T(x) macro for static text.
#define _GTF(c)		c##W		//!< _WIN32 name has a A or W for UTF8 or UNICODE (like _FNF)
#else
	typedef char GChar_t;			//!< My version of TCHAR, _TCHAR
#define _GT(x)		__TOA(x)	//!< like _T(x) macro for static text.
#define _GTF(c)		c##A		//!< _WIN32 name has a A or W for UTF8 or UNICODE (like _FNF)
#endif
