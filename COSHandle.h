...
namespace Gray
{
#define HANDLE_NULL		NULL	//!< Invalid handle for _WIN32. Not invalid OS handle for __linux__.

#define WINHANDLE_NULL	NULL	//!< HWND is NOT OS Handle. like HANDLEPTR_NULL. This is a WINAPI void* handle via DECLARE_HANDLE like HWND, HPEN, HINSTANCE, etc. can't call CloseHandle() on it.
...
#define USE_FILE_POS64
	...
	public:
		explicit inline COSHandle(HANDLE h = INVALID_HANDLE_VALUE)
		: m_h(h)
		{
		}

		COSHandle(const COSHandle& Handle)
		...
		static bool inline IsValidHandle(HANDLE h)
		{
		...
		static inline bool CloseHandle(HANDLE h) // static
		{
			//! default implementation for closing OS HANDLE.
			//! ASSUME IsValidHandle(h)
			//! @return true = ok;
#ifdef _WIN32
			BOOL bRet = ::CloseHandle(h); // ignored bool return.
			return bRet;
		...
		STREAM_SEEKRET_t Seek(STREAM_OFFSET_t lOffset, SEEK_ORIGIN_TYPE eSeekOrigin) const
		{
			...
			bool bRet = ::SetFilePointerEx(m_h, NewFilePointer, &NewFilePointer, eSeekOrigin);
			if (!bRet)
			{
		...
		{
			ASSERT(isValidHandle());
			bool bRet = ::SetHandleInformation(m_h, dwMask, dwFlags);
			return bRet;
		}
#endif
		...
		HANDLE Duplicate(HANDLE hTargetProcess = INVALID_HANDLE_VALUE, DWORD dwDesiredAccess = DUPLICATE_SAME_ACCESS, bool bInheritHandle = false, DWORD dwOptions = DUPLICATE_SAME_ACCESS) const
		{
			...
			bool bRet = ::DuplicateHandle(hCurrentProcess, m_h, hTargetProcess,
