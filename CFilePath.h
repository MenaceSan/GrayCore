...
namespace Gray
{
	...
	public:
		...
		static const FILECHAR_t k_DirSep2 = '/';	//!< preferred for __linux__ NFS but allowed by _WIN32.
#define FILESTR_DirSep		"\\"
#else
		static const FILECHAR_t k_DirSep = '/';		//!< preferred for __linux__ NFS but allowed by _WIN32.
		static const FILECHAR_t k_DirSep2 = '\\';	//!< preferred for _WIN32.
#define FILESTR_DirSep		"/"
#endif
	...
	public:
		static inline bool IsCharDirSep(wchar_t ch)
		{
			//! FAT honors backslash. NTFS honors both chars. NFS uses just the forward slash.
			//! FILECHR_Dirs, like MFC IsDirSep()
			return(ch == k_DirSep || ch == k_DirSep2);
		}
		static inline bool IsCharWildcard(wchar_t ch)
		{
			//! FILECHR_Wildcard
			return (ch == '?' || ch == '*');
		}
