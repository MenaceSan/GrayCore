...
namespace Gray
{
	...
	HWND COSProcess::FindWindowForProcessID(PROCESSID_t nProcessId, DWORD dwStyleFlags, const GChar_t* pszClassName) // static
	{
		...
		{
			return _GTF(::FindWindow)(pszClassName, nullptr);
		}

		// Get the first top window handle. HWND_DESKTOP
		HWND hWnd = _GTF(::FindWindow)(nullptr, nullptr); // ::GetDesktopWindow() ?
		...
		for (; hWnd != WINHANDLE_NULL; hWnd = ::GetWindow(hWnd, GW_HWNDNEXT))
		{
			...
			if (dwStyleFlags)
			{
				DWORD dwStyle = (DWORD)_GTF(::GetWindowLong)(hWnd, GWL_STYLE);
			...
			if (pszClassName != nullptr)	// must be class name.
			{
				...
				int iLen = _GTF(::GetClassName)(hWnd, szClassNameTmp, _countof(szClassNameTmp));
				if (iLen <= 0)
					continue;
