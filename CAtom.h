...
namespace Gray
{
	typedef HASHCODE32_t ATOMCODE_t;	//!< Encode a atom as a hashcode instead of using its name/pointer. StrT::GetHashCode32()
...
#define CATOM_STATSTR(a)	#a	//!< A static atom i know is defined some place, but i just want to use the string here.
	...
	class GRAYCORE_LINK CAtomDef : public CSmartBase
	{
		...
		//! Internal holder for the atom data. Don't use this publicly. Use CAtomRef.
	...
	public:
		...
		const ATOMCHAR_t* get_Name() const
		{
			return m_s;
		}
	};
	...
	private:
		CAtomDefPtr	m_pDef;	//!< NOT allowed to be nullptr
