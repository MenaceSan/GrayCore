...
namespace Gray
{
	struct GRAYCORE_LINK CMem	// static CSingleton
	{
		...
		static inline bool IsZeros(const void* pData, size_t nSize)
		{
	...
	public:
		...
		CMemBlock(size_t nSize, void* pData)
			: m_nSize(nSize)
			, m_pData(pData)
		{
		}
		CMemBlock(size_t nSize, const void* pData)
			: m_nSize(nSize)
			, m_pData(const_cast<void*>(pData))	// just assume we don't modify it?
		{
		}
		CMemBlock(void* pData, size_t nSize)
			: m_nSize(nSize)
			, m_pData(pData)
		{
		}
		CMemBlock(const void* pData, size_t nSize)
			: m_nSize(nSize)
			, m_pData(const_cast<void*>(pData))	// just assume we don't modify it?
		{
		}
		CMemBlock(const CMemBlock& block)
			: m_nSize(block.m_nSize)
			, m_pData(block.m_pData)
		{
			// Just shared pointers. This may be dangerous!
		}
		CMemBlock(const CMemBlock* pBlock)
		...
		bool IsValidIndex2(size_t i) const
		{
...
#define IsEqualLiteral(x) IsEqualData( STRLIT2(x))
		...
		BYTE* get_Start() const
		{
			//! Get as a byte pointer.
			return (BYTE*)m_pData;
		}
		char* get_StartStr() const
		{
			//! Get as a byte pointer.
			return (char*)m_pData;
		}
