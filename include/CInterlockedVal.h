...
/// </summary>
/// <returns>previous value in *pDest</returns>
...
inline INT32 __cdecl InterlockedIncrement(INT32 VOLATILE* pDest) noexcept {
    INT32 lValNew;
#if defined(__GNUC__)
    __asm__ __volatile__("lock; xaddl %0, %1" : "=r"(lValNew), "=m"(*pDest) : "0"(1), "m"(*pDest));
    return (lValNew + 1);
#else
    INT32 nValComp;
    do {
...
template <>
inline INTER32_t Increment<INTER32_t>(INTER32_t VOLATILE* pnValue) noexcept {
    return InterlockedIncrement(pnValue);
}
template <>
inline INTER32_t Decrement<INTER32_t>(INTER32_t VOLATILE* pnValue) noexcept {
    return InterlockedDecrement(pnValue);
}
template <>
inline INTER32_t ExchangeAdd<INTER32_t>(INTER32_t VOLATILE* pnValue, INTER32_t nValue) noexcept {
    return InterlockedExchangeAdd(pnValue, nValue);
}
template <>
inline INTER32_t Exchange<INTER32_t>(INTER32_t VOLATILE* pnValue, INTER32_t nValue) noexcept {
    return InterlockedExchange(pnValue, nValue);
}
template <>
inline INTER32_t CompareExchange<INTER32_t>(INTER32_t VOLATILE* pnValue, INTER32_t nValue, INTER32_t lComparand) noexcept {
    return InterlockedCompareExchange(pnValue, nValue, lComparand);
}

// Implement 64 bit as INT64
template <>
inline INT64 Increment<INT64>(INT64 VOLATILE* pnValue) noexcept {
    return _InterlockedIncrement64(pnValue);
}
template <>
inline INT64 Decrement<INT64>(INT64 VOLATILE* pnValue) noexcept {
    return _InterlockedDecrement64(pnValue);
}
template <>
inline INT64 ExchangeAdd<INT64>(INT64 VOLATILE* pnValue, INT64 nValue) noexcept {
    return _InterlockedExchangeAdd64(pnValue, nValue);
}
template <>
inline INT64 Exchange<INT64>(INT64 VOLATILE* pnValue, INT64 nValue) noexcept {
    return _InterlockedExchange64(pnValue, nValue);
}
template <>
inline INT64 CompareExchange<INT64>(INT64 VOLATILE* pnValue, INT64 nValue, INT64 lComparand) noexcept {
...
#define INTERLOCK_REMAP(T, TI)                                                            \
    template <>                                                                           \
    inline T Increment<T>(T VOLATILE * pnValue) noexcept {                                \
        return (T)Increment<TI>((TI VOLATILE*)pnValue);                                   \
    }                                                                                     \
    template <>                                                                           \
    inline T Decrement<T>(T VOLATILE * pnValue) noexcept {                                \
        return (T)Decrement<TI>((TI VOLATILE*)pnValue);                                   \
    }                                                                                     \
    template <>                                                                           \
    inline T ExchangeAdd<T>(T VOLATILE * pnValue, T nValue) noexcept {                    \
        return (T)ExchangeAdd<TI>((TI VOLATILE*)pnValue, (TI)nValue);                     \
    }                                                                                     \
    template <>                                                                           \
    inline T Exchange<T>(T VOLATILE * pnValue, T nValue) noexcept {                       \
        return (T)Exchange<TI>((TI VOLATILE*)pnValue, (TI)nValue);                        \
    }                                                                                     \
    template <>                                                                           \
    inline T CompareExchange<T>(T VOLATILE * pnValue, T nValue, T lComparand) noexcept {  \
...
class GRAYCORE_LINK cInterlockedVal {
 ...
 public:
    ...
    /// <summary>
    /// Add value. Can be used to set bits but make sure not already set!?
    /// </summary>
...
/// <summary>
/// Used as a thread safe check for code reentrant. even on the same thread. like cLockableBase.
