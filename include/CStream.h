...
#undef CTYPE_DEF
	...
	public:
		...
		virtual HRESULT ReadX(OUT void* pData, size_t nDataSize)
		{
			...
			//! @arg
			//!  pData = nullptr = skip over nDataSize in read stream. Same as SeekX().
			//! @return
...
#undef CTYPE_DEF
	...
	protected:
		HRESULT TransactionRollback()
		{
			...
			return m_pInp->SeekX((STREAM_OFFSET_t)m_lPosStart, SEEK_Set);
		}
	...
	public:
			...
			: m_pInp(pInp)
		{
			...
			ASSERT(m_lPosStart >= 0 && m_lPosStart <= (STREAM_POS_t)cHeap::k_ALLOC_MAX);
