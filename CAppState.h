...
namespace Gray
{
	...
	public:
		...
		{
			//! Is this structure valid in my context?
		...
		APPSTATE_TYPE_ get_AppState() const
		{
			//! use isInCInit() for loading DLL's.
			return m_eAppState;
		}
		static APPSTATE_TYPE_ GRAYCALL GetAppState();
		void put_AppState(APPSTATE_TYPE_ eAppState);
		void InitAppState();

		THREADID_t get_MainThreadId() const
		{
